...
class SnowBot(commands.AutoShardedBot):
    ...
    async def startup_task(self) -> None: 
        ...
        guild_id, channel_id, message_id, variable = [int(var) for var in await self.redis.hmget("restart", "guild_id", "channel_id", "message_id", "to_send")]
        if not variable:
            return 
        elif int(variable) == 0:
            return
...
intents = discord.Intents(members=True, bans=True, guilds=True, messages=True, presences=True, dm_reactions=True, reactions=True)
